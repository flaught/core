...
> **Why this matters.** Agents (and humans) routinely write tests that *cannot fail* — symmetric inputs where a bug and its fix produce identical output, assertions that encode the implementation's current behavior as "correct," and tests that exercise only the happy path. A test that passes identically on pre- and post-change code is not testing the change, regardless of how many such tests exist or whether the suite is green. Test inversion is the structural catch for this pathology. The motivation and the related test-quality scrutiny guidance in the default review prompt are grounded in [Dan Luu's research on how well agents use test and verification techniques](https://danluu.com/agentic-testing/); see the README's [Acknowledgments](../README.md#acknowledgments).
