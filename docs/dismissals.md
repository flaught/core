...
**Mitigation — active dismissals are echoed back into the prompt.** Every LLM review run injects a "Previously Reviewed & Dismissed" digest (title + dismissal reason for each non-expired entry, most recent first, capped at 25) into the user prompt, with an explicit instruction not to re-raise the same underlying issue under new wording. This attacks the root cause instead of the symptom: rather than trying to pattern-match every possible rewording of a dismissed finding after the LLM generates it, it tells the LLM about the dismissal *before* it reviews, so it doesn't generate the restatement in the first place. It's a strong nudge, not a guarantee — a sufficiently different framing, or a model that doesn't follow instructions well, can still slip through, in which case the fingerprint-mismatch failure mode above still applies and the finding needs re-triage. For a class of finding that's *always* a non-issue for a given path (e.g. an ADR that structurally accompanies its own change), prefer [`scope_creep.exclude_paths`](configuration.md#scope-creep-detection) over repeated dismissal — it prevents the finding rather than suppressing it after the fact.
