...
// ─── Report command: render markdown from an existing findings artifact ───────

/**
 * `flaught report` — render a PR-comment markdown report from an existing
 * findings JSON artifact, with no LLM call and no git diff.
 *
 * This closes the CI gap where the PR comment was generated by re-running
 * `flaught review --no-llm` (to avoid a second billed LLM call), which silently
 * dropped every LLM-sourced finding from the comment a human actually reads.
 * With `report`, CI runs the real review once (`flaught review --output
 * findings.json`) and then `flaught report --from findings.json` to produce the
 * comment body from the same artifact — including the LLM findings.
 *
 * Reuses parseArtifactFile (the same schema-validated loader `flaught dismiss`
...
function runReport(argPath: string | undefined, opts: { from?: string }): void {
  ...
  if (!artifactPath) {
    console.error(
      "\n❌ No artifact path given. Usage: `flaught report --from <path>` (or `flaught report <path>`).",
    );
    process.exit(2);
  }
